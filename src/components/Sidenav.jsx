...
import { FaBook, FaCogs, FaCross, FaEye, FaHome, FaPen, FaQuestionCircle } from 'react-icons/fa'
  ...
  return (
    <div className='w-screen bg-slate-300 flex h-screen'>
      <div className='p-2 w-[200px] py-8 bg-white shadow shadow-black h-full flex flex-col justify-between uppercase'>
        <h1 className=' font-light flex items-center text-center w-full'>VICTORY</h1>
        <div className='h-[280px] flex flex-col justify-between px-1 text-slate-700 font-semibold'>
        ...
        <Link to='generate' className='font-bold text-slate-600 hover:translate-x-4 transition items-center flex hover:scale-125 bg-lime-400 p-1 hover:bg-transparent'><FaCogs className='mr-2'/>Generate</Link>
      </div>
      <div className='h-full w-full pl-10 pr-3 pt-10'>
        <Outlet />
      </div>
    </div>
  ) 
